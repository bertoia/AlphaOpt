...
We validate the performance of the acquisition functions and cost-models using the python library. GPyOpt.
Here are the various variations that we considered for the experiments:
...
\begin{figure}[h]
	\includegraphics[width=\linewidth]{PIT_SVM_X_init2_STD_ITER40.png}
	\caption{CNN with Ransom search on 40 iterations}
\end{figure}
The following table gives validation results:
\begin{center}
	\begin{tabular}{lllll}
		\hline
		Cost Model & Acquisition & Best Acc & Best Iter. & Max Iter.\\
		\hline
		\multirow{3}{*}{Std. Cost} & EI &  0.7664 & \textbf{0.2752} &\\
		& EI-EEC & \textbf{0.7634} & \textbf{0.2752} &\\
		& Random & 0.7640 & 0.2724 &\\
		\hline
		\multirow{5}{*}{No Cost} & EI &  0.7664 & \textbf{0.2752} & \\
		& EI-EEC & \textbf{0.7634} & \textbf{0.2752} & \\
		& PI & 0.7667 & 0.2691 & \\
		& MES & 0.7667 & 0.2691 & \\
		& Random & 0.7640 & 0.2724 & \\
...
Standard Cost evaluations are done over a budget of one hour per run. 

\subsection {Support Vector Machine}

\begin{figure}[h]
	\includegraphics[width=\linewidth]{RAND_SVM_X_init2_STD_ITER40.png}
...
\begin{center}
	...
	\end{tabular}
\end{center}
Standard cost evaluations are ran with a 10 minutes budget and no cost evaluations are capped at a maximum of 40 iterations.
...
An alternative to account for the cost is to balance the explore-exploit by considering the remaining time budget which is inspired by simulated annealing.
